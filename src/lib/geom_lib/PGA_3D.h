...
struct Plane3D;  // A plane with the equation: xX + yY + zZ + w = 0 [plane normal is (x,y,z), w indicates distance from
                 // orig.]
struct Motor3D;  // An element which applies rotaton or tranlations, can be composed together [isomorphic to dual
                 // quaternions]
...
struct Plane3D {
    ...
    void print(const char* title = "") { printf("%s - %s\n", title, std::string(*this).c_str()); }
};
...
struct Line3D {
    ...
    Line3D(float x = 0, float y = 0, float z = 0, float mx = 0, float my = 0, float mz = 0)
...
struct Point3D {
    ...
    Point3D scale(float f) { return Point3D(x * f, y * f, z * f); }
    ...
    float distTo(Point3D p) { return sqrt((x - p.x) * (x - p.x) + (y - p.y) * (y - p.y) + (z - p.z) * (z - p.z)); }
    float distToSqr(Point3D p) { return (x - p.x) * (x - p.x) + (y - p.y) * (y - p.y) + (z - p.z) * (z - p.z); }
    float magnitude() { return sqrt(x * x + y * y + z * z); }
...
struct HomogeneousPoint3D {
    ...
    HomogeneousPoint3D scale(float f) { return HomogeneousPoint3D(x * f, y * f, z * f); }

    HomogeneousPoint3D operator-(HomogeneousPoint3D rhs);
    HomogeneousPoint3D operator+(HomogeneousPoint3D rhs);

    float magnitude() { return w; }

    float magnitudeSqr() { return w * w; }
...
struct Motor3D {
    ...
    Motor3D(float s = 1,
            ...
            float xy = 0,
            float wxyz = 0)
        : s(s), wx(wx), wy(wy), wz(wz), yz(yz), zx(zx), xy(xy), wxyz(wxyz) {}
    Motor3D(float s, Line3D l) : s(s), wx(l.mx), wy(l.my), wz(l.mz), yz(l.x), zx(l.y), xy(l.z) {}
    Motor3D(MultiVector mv)
        : s(mv.s), wx(mv.wx), wy(mv.wy), wz(mv.wz), yz(mv.yz), zx(mv.zx), xy(mv.xy), wxyz(mv.wxyz){};
